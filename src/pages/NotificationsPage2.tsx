...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    if (user) {
      try {
        const notificationsJson = localStorage.getItem(`notifications_${user.id}`);
  ...
  return (
    <div className="animate-fade-in" style={{ minHeight: '100vh', padding: '2rem', background: 'var(--background)' }}>
      <button
        onClick={() => navigate(-1)}
        style={{
          background: 'none',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {filteredNotifications.map((notification) => (
              <div
                ...
                style={{
                  background: notification.read ? 'var(--surface)' : '#FFFDF9',
